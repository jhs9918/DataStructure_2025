...
struct State {
    int cost;     // 현재까지의 시간 (첫 번째 필드!)
    int x, y;
    int dir;      // 현재 방향

    // priority_queue용 비교 연산자 (비용이 작은 것이 우선)
...
int bfs() {
    ...
    for (int d = 0; d < 4; d++) {
        ...
        if (nx >= 0 && nx < n && ny >= 0 && ny < n && grid[ny][nx] == 0) {
            pq.push({1, nx, ny, d});  // {cost, x, y, dir} 순서!
            dist[ny][nx][d] = 1;
        }
    }
    ...
    while (!pq.empty()) {
        ...
        for (int newDir = 0; newDir < 4; newDir++) {
            ...
            if (newCost < dist[ny][nx][newDir]) {
                ...
                pq.push({newCost, nx, ny, newDir});  // {cost, x, y, dir} 순서!
            }
        }
    }
